...
//		printf("s %x %d\n", 1,1);
		...
		if (set_minus) {
        setBit(codeword>>1|1, q + 2 + k );

		} else {
        setBit(codeword>>1|0, q + 2 + k );
...
// log2(2),floor(1));
	...
	if (set_minus) {
		  setBit(sum<<1|1, codeword_length+set_bit_size+1);

	} else {
		  setBit(sum<<1|0, codeword_length+set_bit_size+1);
	}
